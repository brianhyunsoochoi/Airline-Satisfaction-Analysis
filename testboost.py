# ===============================================
# Full XGBoost Pipeline — Train + Predict + Kaggle Submission
# ===============================================
...
def generate_submission(model, scaler, feature_cols):
    ...
    # If left as NaN, XGBoost handles it automatically
    X_unknown_scaled = scaler.transform(unknown.fillna(0))  # Scaler does not allow NaN → temporarily convert only to 0
    y_pred = model.predict(X_unknown_scaled)

    y_pred_label = ["satisfied" if p == 1 else "neutral or dissatisfied" for p in y_pred]

    # Keep original id values (no dropped rows)
